...
void aprintf(char* sre, int n, long double a[n]);
long double calculate(int n, long double x[n][2]);
...
int main(int argc, char *argv[]) {
  ...
  long double result = c[3][0] + 2.2L + calculate(n, c);

  cov_log(result, "result", argv[2]); // log.cov
